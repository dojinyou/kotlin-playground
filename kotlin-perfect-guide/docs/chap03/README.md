...
10. 패키지를 사용해 코드를 어떻게 여러 그룹으로 나눌 수 있는 가? 자바와 코틀린 패키지의 가장 핵심적인 차이는 무엇인지 설명하라.
    - 자바와 유사하게 패키지를 통해서 코틀린 역시 코드를 여러 그룹으로 나눌 수 있다.   
      하지만 자바는 실제 파일의 디렉토리 구조와 패키지 구조가 일치해야 하지만 코틀린은 별개로 구성할 수 있다.  
      가령 예를 들어, 같은 패키지의 파일을 다른 디렉토리에서 관리할 수 있으며, 다른 패키지의 파일도 같은 디렉토리에서 관리할 수 있다.  
      하지만 일반적으로 디렉토리 구조와 패키지구조를 일치시키는 걸 권장하고 있다.
11. 임포트 별명이란 무엇인가? 자바의 정적 임포트와 비슷한 임포트를 코틀린에서는 어떻게 처리하는가?
    - 임포트 별명은 다른 패키지에 존재하는 동일한 이름의 선언이 존재하여 한 곳에서 이름으로 식별하여 사용할 수 없을 떄 별명을 통해서 식별하여 사용가능하도록 해준다.  
      자바의 정적 임포트와 다르게 코틀린의 경우에는 일반적인 임포트 디렉티브 구문을 사용해 모든 선언을 임포트할 수 있다.
12. `if` 문/식은 어떤 일을 하는가? 각각을 자바의 if문 및 3항 조건 연산자(?:)와 비교해 보아라
    - 문의 경우에는 특정 행동을, 식의 경우네는 결과를 반환하는 것을 목적으로 하기 때문에  
      `if` 문은 특정 조건에 대한 행동을 정의할 수 있다. `if` 식은 조건별로 다른 결과를 만들 수 있다.  
      또한 식의 경우 모든 경우를 처리해야하기 때문에 `else` 구문을 생략할 수 없다.  
      자바의 `if`문은 문으로만 동작하기 때문에 식으로 활용할 수 없다.   
      `3항 조건 연산자`의 경우 코틀린에는 존재하지 않으며 `if` 식을 이용하여 필요한 곳에서 3항 연산자와 동일하게 사용할 수 있다.
